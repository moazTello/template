...
export default function CircProgress(props) {
  ...
  return (
    <CircularProgressbarWithChildren value={percentage} text={`${percentage}%`} styles={buildStyles(stylesColorMode)}>
      <Box>
        <Text fontSize="sm" color="secondaryGray.600" fontWeight="500">
          {title}
        </Text>
        <Text fontSize="xl" textAlign="center" color={textColor} fontWeight="700">
          {percentage}%
        </Text>
      </Box>
