...
export function SidebarLinks(props) {
  ...
  const activeRoute = (routeName) => {
    if (location.pathname.includes('store') && !routeName.name.toLowerCase().includes('store')) return false;
    return location.pathname.includes(routeName.path.toLowerCase());
  };
  ...
  const createLinks = (routes) => {
    return routes.map((route, key) => {
      ...
      if (route.collapse) {
        if (!hasPermission(getAllPermissionKeys(route.items), user?.role?.permissions)) return;
        return (
              ...
              >
                {route.icon ? (
                  <Flex
                    align="center"
                    justifyContent={
                      mini === false ? 'space-between' : mini === true && hovered === true ? 'space-between' : 'center'
                    }
                    w="100%"
                  >
                    <HStack mb="6px" spacing={activeRoute(route) ? '22px' : '26px'}>
        ...
        if (!hasPermission(route.permission, user?.role?.permissions)) return;
        return (
          <NavLink to={route.layout + route.path} key={key}>
            {route.icon ? (
              <Flex align="center" justifyContent="space-between" w="100%" ps="17px" mb="0px">
                <HStack mb="6px" spacing={activeRoute(route) ? '22px' : '26px'}>
                  <Flex w="100%" alignItems="center" justifyContent="center">
                    <Box color={activeRoute(route) ? activeIcon : inactiveColor} me="12px" mt="6px">
                      {route.icon}
                    </Box>
                    <Text me="auto" color={activeRoute(route) ? activeColor : 'secondaryGray.600'} fontWeight="500">
                      {mini === false ? route.name : mini === true && hovered === true ? route.name : route.name[0]}
                    </Text>
                  </Flex>
                </HStack>
              ...
              <ListItem ms={null}>
                <Flex
                  ...
                  mb="8px"
                >
                  <Text color={activeRoute(route) ? activeColor : inactiveColor} fontWeight="500" fontSize="sm">
                    {mini === false ? route.name : mini === true && hovered === true ? route.name : route.name[0]}
                  </Text>
                </Flex>
              </ListItem>
